...
const PendingRMARequests = ({ rmaRequests }) => {
  ...
  // Ensure rmaRequests is always an array
  const safeRMARequests = Array.isArray(rmaRequests) ? rmaRequests : [];
  ...
  const filteredRMARequests = safeRMARequests
  ...
  return (
    <div className="w-[35%] h-full flex gap-4">
      <div
        className={`border border-gray-200 rounded-lg shadow-lg w-full h-full ${
          darkMode ? 'bg-light-container' : 'dark:bg-dark-container'
        }`}
      >
        <div className="w-full px-4 py-2 flex items-center justify-start">
          <h2 className={`font-semibold text-lg ${darkMode ? 'text-light-primary' : 'text-dark-primary'}`}>
            Pending RMA Requests
          </h2>
        </div>
        <div
          className={`w-full p-2 flex items-center justify-center h-[90%] rounded-lg ${
            darkMode ? 'bg-light-container text-light-textPrimary' : 'bg-dark-container text-dark-textPrimary'
          }`}
        >
              ...
              {filteredRMARequests.length > 0 ? (
                filteredRMARequests.map((item, index) => (
                  <tr key={index}>
                    ...
                    >
                      <span
                        className={`${getStatusStyles(item.status).textClass} ${getStatusStyles(item.status).bgClass} py-1 px-3 rounded-md`}
                      >
                        {item.status}
                      </span>
                    </td>
